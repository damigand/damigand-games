...
import { drawScore, updateScore, loadScore, crossScore, circleScore } from '~c/ScoreComponent/score.js';
...
export function threeInRowGame() {
    ...
    return `
        <article class="game-threeinrow spacing">
            ${drawScore('threeinrow', scoreHTML())}
...
function finishGame(winner) {
    ...
    if (winner == 1) {
        ...
        const result = updateScore(gameEnum.THREEINROW, true);
        score.textContent = result;
    } else {
        var score = document.querySelector('.player-1 p');
        const result = updateScore(gameEnum.THREEINROW, false);
        score.textContent = result;
    }
